...
void mqtt_init(mqtt_broker_handle_t* broker, const char* clientid) {
    ...
    if(clientid) {
        ...
        strncpy(broker->clientid, "emqtt", sizeof(broker->clientid));
    }
    // Will topic
...
int mqtt_connect(mqtt_broker_handle_t* broker)
{
    ...
    int ret = broker->sendBuf(broker->socket_info, packet, offset);
    Free(packet);
    if(ret != offset) {
        return OPRT_MID_MQTT_PACK_SEND_FAILED;
    }
...
int mqtt_disconnect(mqtt_broker_handle_t* broker) {
    ...
    // Send the packet
    if(broker->sendBuf(broker->socket_info, packet, sizeof(packet)) != sizeof(packet)) {
        return -1;
    }
...
void mqtt_get_qos_msg_id(mqtt_broker_handle_t* broker, uint8_t qos, uint16_t* message_id)
{
...
int mqtt_pub_msg_with_qos(mqtt_broker_handle_t* broker, const char* topic, const unsigned char* msg, const uint32_t m_len,uint8_t retain, uint8_t qos, uint16_t message_id)
{
    uint16_t topiclen = strlen(topic);
    uint32_t msglen = m_len;
    ...
    uint16_t var_header_len = topiclen+2+qos_size;
    uint8_t *var_header = Malloc(var_header_len);
    if(NULL == var_header) {
        return -1;
    }
    ...
    // uint8_t fixed_header[fixedHeaderSize];
    uint8_t *fixed_header = Malloc(fixedHeaderSize);
    if(NULL == fixed_header) {
        Free(var_header);
        return -2;
    }
    ...
    length = remainLen;
    do {
        fixed_header[offset] = length % 128;
        length >>= 7;
        if (length > 0) {
            fixed_header[offset] |= 0x80;
            offset++;
        }
    } while (length > 0);

    // uint8_t packet[sizeof(fixed_header)+var_header_len+msglen];
    uint32_t packet_len = fixedHeaderSize+var_header_len+msglen;
    ...
    Free(var_header);
    // Send the packet
    if(broker->sendBuf(broker->socket_info, packet, packet_len) != packet_len) {
        Free(packet);
        return OPRT_MID_MQTT_PACK_SEND_FAILED;
    }
    Free(packet);
...
{
    return mqtt_pub_msg_with_qos(broker, topic, (unsigned char *)msg, strlen(msg),retain, qos, message_id);
}
#endif
...
int mqtt_subscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) {
    ...
    //    utf_topic[topiclen+2] = MQTT_QOS1_FLAG;
